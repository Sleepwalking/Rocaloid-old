'    一些公用变量，常数和函数，在哪都可能会用到
'    Some common variables, constants and subs, might be used anywhere.
...
Friend Module OverAll
	...
	Public Sub Swap(Of T)(ByRef S1 As T, ByRef S2 As T)
		...
		S2 = Temp
	End Sub
	Public Function GetDoubleSum(ByRef Arr() As Double) As Double
		Dim Acc As Double
		Dim i As Double
		For Each i In Arr
			Acc += i
	...
	Public Function GetFreqByPitch(ByVal Pitch As String) As Double
		...
		Return 0
	End Function
	Public Function GetPitchByFreq(ByVal Freq As Double) As String
		Dim i As Integer
		For i = 0 To FreqList.Length - 1
			If (FreqList(i) + FreqList(i + 1)) / 2 <= Freq AndAlso (FreqList(i + 1) + FreqList(i + 2))/2 > Freq Then
				Return PitchList(i + 1)
			End If
		Next
		If Freq < (FreqList(0) + FreqList(1))/2 Then Return PitchList(0)
		Return PitchList(PitchList.Length)
	End Function
